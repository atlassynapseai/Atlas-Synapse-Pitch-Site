...
/** Opaque sticky-note colors: each card is distinct; text uses fixed dark ink for readability */
const items = [
  {
  ...
  return (
    ...
    >
      <svg className="pointer-events-none absolute inset-0 h-full w-full" aria-hidden="true">
        <polygon points="0,0 450,30 350,380 0,340" fill="rgba(99,102,241,0.06)" stroke="rgba(99,102,241,0.1)" strokeWidth="1.5" />
        <polygon points="700,0 1440,50 1440,420 650,370" fill="rgba(45,27,105,0.04)" stroke="rgba(45,27,105,0.08)" strokeWidth="1.5" />
        <polygon points="300,200 700,160 800,450 250,480" fill="rgba(217,119,6,0.05)" stroke="rgba(180,100,40,0.08)" strokeWidth="1" />
        <polygon points="950,150 1440,100 1440,400 900,380" fill="rgba(129,140,248,0.05)" stroke="rgba(129,140,248,0.09)" strokeWidth="1" />
        <polygon points="80,320 380,290 450,520 50,550" fill="rgba(120,100,80,0.05)" stroke="rgba(90,70,55,0.07)" strokeWidth="1" />
        <polygon points="500,0 800,60 720,280 440,240" fill="rgba(167,139,250,0.05)" stroke="rgba(129,140,248,0.08)" strokeWidth="1" />
      </svg>
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1400px]">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: E }}
          ...
          <p className="text-[14px] font-extrabold uppercase tracking-[0.12em] text-[#6D28D9]">What we replace</p>
          <h2 className="mt-2 text-[40px] font-extrabold tracking-[-0.02em] text-[#1B1464] md:text-[54px] lg:text-[62px]">
            The <span className="text-[#5B21B6]">AI Management</span> Stack
          </h2>
          <p className="mt-2 max-w-[500px] text-[17px] leading-[1.6] text-[#27272A] md:text-[18px]">
            Everything <span className="font-bold text-[#6D28D9]">HR</span> does for your human team. We do the same for your AI agents.
          </p>
        </motion.div>
          ...
          <div className="grid grid-cols-3 gap-5">
            {items.slice(0, 3).map((item, i) => (
              <motion.div key={i}
                ...
                className="rounded-2xl border-2 px-5 py-5 shadow-[0_12px_32px_rgba(0,0,0,0.12)]"
                style={{
                  background: item.bg,
              ...
              >
                ...
                <p className="text-[19px] font-bold leading-tight" style={{ color: item.titleColor }}>{item.label}</p>
                <p className="mt-1.5 text-[13px] font-medium leading-snug" style={{ color: item.subColor }}>{item.sub}</p>
              </motion.div>
            ))}
          </div>
        ...
        <div className="space-y-3 md:hidden">
          {items.map((item, i) => (
            <motion.div key={i}
              ...
              className="rounded-2xl border-2 px-5 py-4 shadow-[0_10px_28px_rgba(0,0,0,0.1)]"
            ...
            >
              ...
              <p className="text-[17px] font-bold leading-tight" style={{ color: item.titleColor }}>{item.label}</p>
              <p className="mt-1 text-[13px] font-medium leading-snug" style={{ color: item.subColor }}>{item.sub}</p>
            </motion.div>
          ))}
        </div>

        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }}
          className="mt-10 text-center text-[19px] italic leading-relaxed text-[#3F3A45] lg:text-[21px]">
